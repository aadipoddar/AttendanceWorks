...
public partial class MainPage : ContentPage
{
			...
			if (location is not null)
			{
				...
				if (_currentClass is not null)
				{
					...
					if (classRoom is not null)
					{
						...
						_isInClassRange = differenceKilometeres < 0.015;
	...
	private async Task MarkAttendance()
	{
		...
		// Check if we're in the 5-minute window in the middle of the lecture
		if (!IsWithinAttendanceWindow())
		{
			await DisplayAlert("Attendance Window Closed",
				"Attendance can only be marked during the 5-minute window in the middle of the lecture.", "OK");
			return;
		}
	...
	/// <summary>
	/// Checks if the current time is within the 5-minute window in the middle of the lecture
	/// </summary>
	private bool IsWithinAttendanceWindow()
	{
		if (_currentClass is null)
			return false;

		// Convert TimeOnly to DateTime for calculations
		var now = DateTime.Now.TimeOfDay;
		var startTime = _currentClass.StartTime.ToTimeSpan();
		var endTime = _currentClass.EndTime.ToTimeSpan();

		// Calculate the middle of the class period
		var totalDuration = (endTime - startTime).TotalMinutes;
		var middlePoint = startTime.Add(TimeSpan.FromMinutes(totalDuration / 2));

		// Define the 5-minute window (2.5 minutes before and 2.5 minutes after the middle point)
		var windowStart = middlePoint.Add(TimeSpan.FromMinutes(-2.5));
		var windowEnd = middlePoint.Add(TimeSpan.FromMinutes(2.5));
	...
	private async void ViewHistoryButton_Clicked(object sender, EventArgs e) =>
		await Navigation.PushAsync(new AttendanceReportPage(_student));
	...
	private async Task CreateClassNotifications()
	{
		...
		foreach (var classItem in classes)
		{
			...
			// Calculate the attendance window time (middle of class)
			var classDuration = (classItem.EndTime.ToTimeSpan() - classItem.StartTime.ToTimeSpan()).TotalMinutes;
			var middlePoint = classDateTime.AddMinutes(classDuration / 2);
			var attendanceWindowStart = middlePoint.AddMinutes(-2.5);

			// Create notification for the attendance window
			var attendanceWindowNotification = new NotificationRequest
			{
				NotificationId = classItem.Id + 20000,
				Title = $"Attendance Window Opening",
				Subtitle = $"{classItem.CourseCode}: Attendance Time",
				Description = $"The 5-minute attendance window for {classItem.CourseName} is now open. Please mark your attendance.",
				CategoryType = NotificationCategoryType.Alarm,
				ReturningData = classItem.ClassRoomId.ToString(),
				Schedule = new NotificationRequestSchedule
				{
					NotifyTime = attendanceWindowStart
				}
			};

			await LocalNotificationCenter.Current.Show(notification);
			await LocalNotificationCenter.Current.Show(startNotification);
			await LocalNotificationCenter.Current.Show(attendanceWindowNotification);
		}
	}
